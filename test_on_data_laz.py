"""
Тестирование обученной PointNet Segmentation на файле data.laz
...
print("=" * 80)
print("ТЕСТИРОВАНИЕ POINTNET SEGMENTATION НА DATA.LAZ")
print("=" * 80)
...
def main():
    ...
    try:
        ...
        # НОВОЕ: Попытка загрузить ground truth labels
        gt_labels = None
        for field_name in ['label', 'classification', 'class', 'scalar_Classification']:
            if hasattr(las, field_name):
                gt_labels = np.array(getattr(las, field_name), dtype=np.int32)
    ...
    # 5. НОВОЕ: Расчет метрик качества
